...
class CTCPrefixScorer_free(BatchPartialScorerInterface):
    ...
    def init_state(self, x: torch.Tensor,hand_matrix=None):
    ...
    def batch_init_state(self, x: torch.Tensor,hand_matrix=None):
        """Get an initial state for decoding.
        ...
        if hand_matrix is not None:
            logp = self.ctc.log_softmax(x.unsqueeze(0),hand_matrix)

        else:
            logp = self.ctc.log_softmax(x.unsqueeze(0)).detach()
