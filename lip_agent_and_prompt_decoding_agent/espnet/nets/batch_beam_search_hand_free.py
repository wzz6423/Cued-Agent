...
class BatchBeamSearch_hand_free(BeamSearch):
    ...
    def init_hyp(self, x: torch.Tensor,hand_matrix=None) -> BatchHypothesis:
        ...
        for k, d in self.scorers.items():
            if k=='ctc' and hand_matrix is not None:
                init_states[k] = d.batch_init_state(x,hand_matrix)
            else:
    ...
    def forward(
            self, x: torch.Tensor, maxlenratio: float = 0.0, minlenratio: float = 0.0, hand_matrix=None
    ) -> List[Hypothesis]:
        """Perform beam search.
        ...
        if hand_matrix is not None:
            running_hyps = self.init_hyp(x,hand_matrix)
        else:
